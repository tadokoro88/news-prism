...
## Phase 3: Web UI のデプロイ (`news-prism.korotora.com`)

Route53 hosted zone が **別 account (mgmt)** にあるため、DNS レコードは Terraform で
扱わず手動で投入する。手順:
...
`acm_validation_records` で出る `name` / `type` / `value` を **mgmt account の Route53
`korotora.com` hosted zone に手動で CNAME 投入**する。1 件のみ (`_xxx.news-prism.korotora.com.` → `_yyy.acm-validations.aws.`)。
...
mgmt の `korotora.com` zone に以下の **A レコード (Alias)** を手動追加:

| Name | Type | Alias to | Hosted zone ID |
|---|---|---|---|
| `news-prism.korotora.com` | A | `${cloudfront_domain_name}` | `Z2FDTNDATAQYW2` |
...
```bash
curl https://news-prism.korotora.com/             # index.html が返る
curl https://news-prism.korotora.com/api/analyze  # 405 (POST のみ許容)
```

ブラウザで `https://news-prism.korotora.com/` を開いて URL 入力 → 解析。
